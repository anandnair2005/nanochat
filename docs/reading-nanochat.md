...
A reading path through Andrej Karpathy's [NanoChat](https://github.com/karpathy/nanochat),
written while working through it. It assumes working Python, basic PyTorch, and
familiarity with transformers, and it pays particular attention to how the repo
handles distributed training across multiple GPUs.

This is the orientation I wish I had on day one. The blog series it led to is
linked from the repository README.

## What the repo contains

nanochat is a compact end-to-end LLM training stack, covering:
...
The strongest recommendation is to spend the most time on `nanochat/optim.py`, because it exposes the distributed communication primitives directly and is more educational than a standard DDP wrapper example.
